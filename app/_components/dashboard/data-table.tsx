...
export function DataTable({
  ...
  const visible = useMemo(() => {
    return getVisibleRows({ columns, rows, searchKeys, filterKey, filter, query, sort });
  ...
  function renderCell(row: DataTableRow, col: ColumnSpec) {
    ...
    switch (col.kind) {
      case "primary": {
        const dot = col.dotColorKey ? String(row[col.dotColorKey] ?? "") : "";
        const inner = (
          <>
            <span
            ...
            <span className="app-dt-primary-body">
              ...
              {col.subKey && row[col.subKey] != null && (
                <span className="app-dt-primary-sub">{String(row[col.subKey])}</span>
              )}
            </span>
          </>
        );
