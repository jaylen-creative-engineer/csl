...
Given("a league {string} with host {string}", async function (this: CslWorld, leagueName: string, hostName: string) {
  const host = await this.leagueModel.createLeagueHost({ name: hostName, organization: "Default Org" });
...
Given("an open challenge {string}", async function (this: CslWorld, title: string) {
...
Given("a participant {string} with discipline {string}", async function (this: CslWorld, handle: string, discipline: string) {
...
Given(
  ...
  async function (this: CslWorld, state: string, count: number) {
    const host = await this.leagueModel.createLeagueHost({ name: "Jordan", organization: "Design Chicago" });
    const league = await this.leagueModel.createLeague({ name: "Test League", hostId: host.id });
    this.currentLeagueId = league.id;
...
Given("a challenge in {string} state", async function (this: CslWorld, state: string) {
  const host = await this.leagueModel.createLeagueHost({ name: "Jordan", organization: "Design Chicago" });
  const league = await this.leagueModel.createLeague({ name: "Test League", hostId: host.id });
  this.currentLeagueId = league.id;
  ...
  if (state === "open") {
    ...
    await this.challengeService.openChallenge(challenge.id);
    await this.challengeService.closeForJudging(challenge.id);
    await this.challengeService.completeChallenge(challenge.id);
  }
...
When(
  ...
  async function (this: CslWorld, title: string, hours: number) {
    const challenge = await this.challengeService.createChallenge({
...
When("the host opens the challenge for submissions", async function (this: CslWorld) {
  await this.challengeService.openChallenge(this.currentChallengeId);
});

When("{string} submits an artifact URL {string}", async function (this: CslWorld, _handle: string, url: string) {
  this.lastError = undefined;
  try {
    this.lastSubmission = await this.challengeService.submitEntry(this.currentChallengeId, this.currentParticipantId, {
      artifact: { url },
      isPublic: true,
    });
  } catch (err) {
...
When("a judge scores all submissions", async function (this: CslWorld) {
  let scoreValue = 90;
...
When("a participant attempts to submit", async function (this: CslWorld) {
  this.lastError = undefined;
  const participant = await this.leagueModel.createParticipant({
    handle: "attempter",
    discipline: Discipline.Design,
  ...
  try {
    await this.challengeService.submitEntry(this.currentChallengeId, participant.id, {
...
Then("the challenge status is {string}", async function (this: CslWorld, expectedStatus: string) {
...
Then("participants can submit entries", async function (this: CslWorld) {
  const challenge = await this.challengeService.getChallenge(this.currentChallengeId);
  assert.ok(challenge, "Expected challenge to exist");
...
Then("the leaderboard shows {string}", async function (this: CslWorld, handle: string) {
  await this.challengeService.closeForJudging(this.currentChallengeId);
  await this.challengeService.scoreSubmission(this.lastSubmission!.id, {
    judgeId: "judge:1",
  ...
  const participant = await this.leagueModel.getParticipant(this.currentParticipantId);
...
Then("the leaderboard is sorted by score descending", async function (this: CslWorld) {
  const leaderboard = await this.challengeService.getLeaderboard(this.currentChallengeId);
  assert.ok(leaderboard.length > 0, "Leaderboard should not be empty");
...
Then("scoring is deterministic for equal inputs", async function (this: CslWorld) {
  const leaderboard1 = await this.challengeService.getLeaderboard(this.currentChallengeId);
  const leaderboard2 = await this.challengeService.getLeaderboard(this.currentChallengeId);
